...
contract ZeroSumSpectator is ReentrancyGuard, Ownable {
    ...
    function claimBettingWinnings(address _gameContract, uint256 _gameId) external nonReentrant {
        ...
        for (uint i = 0; i < bets.length; i++) {
            if (bets[i].bettor == msg.sender && 
                bets[i].predictedWinner == actualWinner && 
                !bets[i].claimed) {
    ...
    function getBettingOdds(address _gameContract, uint256 _gameId, address[] memory _players) external view returns (
        uint256[] memory betAmounts,
        uint256[] memory oddPercentages
    ) {
    ...
    function getGameBettingInfo(address _gameContract, uint256 _gameId) external view returns (
        ...
        bytes32 gameKey = _getGameKey(_gameContract, _gameId);
        return (
            totalGameBets[gameKey],
            gameBets[gameKey].length,
            isBettingAllowed(_gameContract, _gameId)
        );
    }
