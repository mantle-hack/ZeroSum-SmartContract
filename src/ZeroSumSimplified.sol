...
contract ZeroSumSimplified is ReentrancyGuard, Ownable {
    ...
    using SimplifiedLibrary for Game;
    ...
    function createQuickDraw() external payable notPaused {
        ...
        SimplifiedLibrary._createGame(
            GameMode.QUICK_DRAW, gameCounter, games, playerTimeouts, isInGame, gamePlayers, played, turnDeadlines
        );
    }
    ...
    function createStrategic() external payable notPaused {
        require(msg.value > 0, "Fee required");
        SimplifiedLibrary._createGame(
            GameMode.STRATEGIC, gameCounter, games, playerTimeouts, isInGame, gamePlayers, played, turnDeadlines
        );
    }

    // function _createGame(GameMode _mode) internal {
    //     uint256 id = gameCounter++;
    //     SimplifiedLibrary._createGame(_mode, id, games, playerTimeouts);
    ...
    function _joinGame(uint256 _id, address _player) internal {
        ...
        SimplifiedLibrary._joinGame(_id, _player, games, isInGame, gamePlayers, played, playerTimeouts, turnDeadlines);
    }
    ...
    function _startGame(uint256 _id) internal {
        ...
        SimplifiedLibrary._startGame(_id, games, gamePlayers, turnDeadlines);
    ...
    function getTimeoutStatus(uint256 _id, address _player)
        external
        view
        returns (uint256 timeouts, uint256 remaining)
    {
    ...
    function verifyFairness(uint256 _id) external view returns (bool, uint256, string memory) {
        Game memory g = games[_id];
        return (g.numberGenerated, gamePlayers[_id].length, "Both players + block data + custom salt");
    }
